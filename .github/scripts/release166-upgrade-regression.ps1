...
$baseline = Join-Path $repoRoot 'baseline165'
$oldZip = Join-Path $baseline 'codea-harness-1.6.5-windows-x64-install.zip'
$oldChecklist = Get-Content (Join-Path $baseline 'codea-harness-1.6.5-release-checklist.json') -Raw | ConvertFrom-Json
$oldHead = '80d6a7e52eae031e01077862344d6685b252a1ee'
if ($oldChecklist.exactHeadSha -cne $oldHead -or [string]$oldChecklist.workflowRunId -ne '34923142839') { throw '1.6.5 baseline provenance mismatch' }
if ((Get-FileHash $oldZip -Algorithm SHA256).Hash.ToLowerInvariant() -cne [string]$oldChecklist.artifacts.install.sha256) { throw '1.6.5 install ZIP hash mismatch' }
$upgradeZip = Join-Path $repoRoot 'codea-harness-1.6.6-windows-x64-upgrade.zip'
$head = (git -C $repoRoot rev-parse HEAD).Trim()
$project = Join-Path $env:RUNNER_TEMP ('Codea 166 upgrade & spaces ' + [guid]::NewGuid().ToString('N'))
$evidence = [ordered]@{ status='PENDING'; exactHeadSha=$head; baselineRunId='34923142839'; baselineInstallArtifactId='10379038032'; baselineChecklistArtifactId='10379008371'; baselineHead=$oldHead; fromVersion='1.6.5'; toVersion='1.6.6' }
...
try {
    ...
    if ([string]$oldManifest.version -cne '1.6.5' -or [string]$oldManifest.buildCommit -cne $oldHead) { throw 'baseline manifest mismatch' }
    ...
    if ([string]$newManifest.version -cne '1.6.6' -or [string]$newManifest.buildCommit -cne $head) { throw 'candidate manifest mismatch' }
    ...
    if ($oldToolHash -ceq [string]$newManifest.hostAgents.reviewer.submissionToolSha256) { throw '1.6.6 submission tool must differ from the published 1.6.5 tool' }
    ...
    Write-Output 'RELEASE166_PACKAGED_HOST_CONFLICT PASS'
    ...
    if ($upgradeExit -ne 0 -or $result.status -cne 'UPGRADED' -or $result.fromVersion -cne '1.6.5' -or $result.toVersion -cne '1.6.6') { throw "upgrade failed: $raw" }
    ...
    if ($installed.buildCommit -cne $head -or $installed.version -cne '1.6.6') { throw 'installed provenance mismatch' }
    ...
    Write-Output 'RELEASE166_REAL_165_TO_166_UPGRADE PASS'
    Write-Output 'RELEASE166_INSTALLED_RUNTIME_HOST_STATE PASS'
    ...
    [IO.File]::WriteAllText((Join-Path $repoRoot 'codea-harness-1.6.6-upgrade-evidence.json'),($evidence | ConvertTo-Json -Depth 12),$utf8)
    Write-Output 'RELEASE166_SAME_VERSION_NOOP PASS'
} finally {
