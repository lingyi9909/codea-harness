...
foreach ($required in @($runtimeSource, $astGrepSource, $schemaSource)) {
    if (-not (Test-Path $required -PathType Leaf)) {
        throw "Task 1 real regression missing required file: $required"
    }
}

$fixture = Join-Path $env:RUNNER_TEMP ("task153-entrypoint-inventory-" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    try {
        Invoke-Git init
        Invoke-Git config user.email 'task153@example.test'
        Invoke-Git config user.name 'Task 153 Real Regression'
