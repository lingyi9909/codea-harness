...
function Require([bool]$Condition, [string]$Name) {
    if ($Condition) { Write-Output "$Name PASS"; return }
    $failures.Add($Name)
}
...
Require ($task2E2E -match [regex]::Escape('REVIEWER_UNAVAILABLE_FAIL_CLOSED PASS') -and $task2E2E -match [regex]::Escape("Remove-Item (Join-Path `$negative '.opencode/agents/reviewer.md')")) 'MATRIX_REVIEWER_REGISTRATION_MISSING_FAIL_CLOSED'
...
Require ($plainServer -match 'opencode run --command harness-review-reviewer' -and $plainE2E -match '"subagent_type":"reviewer"') 'PACKAGED_TOP_LEVEL_INDEPENDENT_REVIEWER_CHILD'
