...
# version; all package cleanup, binary pins and Host registration stay shared.
...
if ($version -ne '1.6.5') { throw "Expected 1.6.5, got $version" }
$adapted = (Get-Content $source -Raw).Replace('1.6.4', '1.6.5')
$temp = Join-Path $PSScriptRoot ('.release165-package-' + [guid]::NewGuid().ToString('N') + '.tmp.ps1')
try {
    [IO.File]::WriteAllText($temp, $adapted, [Text.UTF8Encoding]::new($false))
    & pwsh -NoProfile -File $temp
    if ($LASTEXITCODE -ne 0) { throw "1.6.5 package build failed: $LASTEXITCODE" }
} finally {
