...
4. When prepare auto-selects one complete chain, or select succeeds, use the returned `scope.reads` as the authority for source reading. Review only that selected scope. Do not add files or ranges that are not present in the scope.
...
6. Always call `codea-review` with `action=finish`, even when findings are empty. Send every actual read, findings, pending risks, and gaps. Only the finish result may authorize the words “评审完成”.
