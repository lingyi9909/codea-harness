...
func certifyWithRuntime153(root string, req CertifyRequest, runtime certificationRuntime153) (Certificate, error) {
	...
	if err != nil {
		return Certificate{}, fmt.Errorf("ANALYSIS_DRAFT_READ_FAILED: %w", err)
	}
	...
	if err := schema.ValidateJSON(analysisSchema, draftBytes); err != nil {
		return Certificate{}, fmt.Errorf("ANALYSIS_DRAFT_SCHEMA_INVALID: %w", err)
	}

	var meta certifyDraftMeta153
	if err := json.Unmarshal(draftBytes, &meta); err != nil {
		return Certificate{}, fmt.Errorf("ANALYSIS_DRAFT_DECODE_FAILED: %w", err)
	}
	var typed ChangeAnalysis
	if err := json.Unmarshal(draftBytes, &typed); err != nil {
		return Certificate{}, fmt.Errorf("ANALYSIS_DRAFT_DECODE_FAILED: %w", err)
	}
	if meta.ReviewScope.BaseRef != req.BaseRef || meta.ReviewScope.IncludeWorkingTree != req.IncludeWorkingTree {
	...
	if err != nil {
		return Certificate{}, fmt.Errorf("ANALYSIS_CANONICALIZE_FAILED: %w", err)
	}
	...
	// Publish the certificate last. A consumer can never observe a new certificate
	// that points at analysis/inventory bytes which have not already been written.
	if err := atomicWriteCertified153(filepath.Join(analysisDir, "change-analysis.json"), canonicalAnalysis); err != nil {
		return Certificate{}, err
	}
	if err := atomicWriteCertified153(filepath.Join(analysisDir, "entrypoint-inventory.json"), inventoryBytes); err != nil {
		return Certificate{}, err
	}
	if err := atomicWriteCertified153(filepath.Join(analysisDir, "change-analysis.cert.json"), certBytes); err != nil {
		return Certificate{}, err
	}
	return cert, nil
}
...
func compareDraftChangeSet153(draft []struct {
	...
	canonSources := func(in []changeset.Source) string {
		...
		out := make([]string, 0, len(copyIn))
		last := changeset.Source("")
		for _, source := range copyIn {
			if source == last { continue }
			last = source
	...
	for _, f := range draft {
		...
		if !ok {
			return fmt.Errorf("CHANGE_SET_MISMATCH: invalid draft path %q", f.Path)
		}
		if _, exists := draftSet[p]; exists {
			return fmt.Errorf("CHANGE_SET_MISMATCH: duplicate draft path %q", p)
		}
	...
	for _, f := range actual {
		p, ok := safeEvidencePath153(f.Path)
		if !ok {
			return fmt.Errorf("CHANGE_SET_MISMATCH: invalid runtime path %q", f.Path)
		}
		actualSet[p] = entry{path: p, sources: canonSources(f.Sources)}
	}
	if len(draftSet) != len(actualSet) {
		return fmt.Errorf("CHANGE_SET_MISMATCH: draft files=%d runtime files=%d", len(draftSet), len(actualSet))
	}
	for p, expected := range actualSet {
		got, ok := draftSet[p]
		if !ok || got.sources != expected.sources {
			return fmt.Errorf("CHANGE_SET_MISMATCH: %s draft sources=%q runtime sources=%q", p, got.sources, expected.sources)
		}
	}
	return nil
}
...
func sameRunDraftPath153(value, runID string) bool {
	...
	return clean == ".code-harness/runs/"+runID+"/requests/change-analysis-draft.json"
}
...
func hashBytes153(data []byte) string {
...
func atomicWriteCertified153(target string, data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("ANALYSIS_ARTIFACT_TEMP_FAILED: %w", err)
	}
	tmpName := tmp.Name()
	defer os.Remove(tmpName)
	if _, err := tmp.Write(data); err != nil {
		_ = tmp.Close()
		return fmt.Errorf("ANALYSIS_ARTIFACT_WRITE_FAILED: %w", err)
	}
	...
	if err := os.Rename(tmpName, target); err != nil {
		return fmt.Errorf("ANALYSIS_ARTIFACT_PUBLISH_FAILED: %w", err)
	}
	return nil
}
