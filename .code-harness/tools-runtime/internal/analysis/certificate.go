...
func loadCertifiedWithRuntime153(root, analysisPath string, runtime certificationRuntime153) (ChangeAnalysis, Certificate, error) {
	...
	snapshotAbs := filepath.Join(analysisDir, "change-set.json")
	...
	// A canonical run publishes change-set.json before certification. Once that
	// Runtime-owned artifact exists, removing canonical identity fields from the
	// certificate must never downgrade the loader to the weaker retained/legacy
	// authority path. Legacy runs predate change-set.json and remain readable.
	_, snapshotStatErr := os.Stat(snapshotAbs)
	snapshotExists := snapshotStatErr == nil
	if snapshotStatErr != nil && !os.IsNotExist(snapshotStatErr) {
		return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CHANGE_SET_SNAPSHOT_STAT_FAILED: %w", snapshotStatErr)
	}
	canonicalIdentityPresent := cert.SnapshotSHA256 != "" || cert.ResolvedBaseCommit != "" || cert.MergeBase != "" || cert.CurrentBranch != ""
	if canonicalIdentityPresent && (cert.SnapshotSHA256 == "" || cert.ResolvedBaseCommit == "" || cert.MergeBase == "" || cert.CurrentBranch == "") {
		return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CERTIFICATE_IDENTITY_MISMATCH: canonical snapshot identity incomplete")
	}
	if snapshotExists && cert.SnapshotSHA256 == "" {
		return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CERTIFICATE_AUTHORITY_MODE_DOWNGRADE")
	}
	...
	if cert.SnapshotSHA256 != "" {
		...
		snapshotBytes, err := os.ReadFile(snapshotAbs)
		if err != nil {
			return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CHANGE_SET_SNAPSHOT_READ_FAILED: %w", err)
		}
		if schemaBytes, readErr := os.ReadFile(filepath.Join(root, ".code-harness", "contracts", "change-set.schema.json")); readErr == nil {
			if err := schema.ValidateJSON(schemaBytes, snapshotBytes); err != nil {
				return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CHANGE_SET_SNAPSHOT_SCHEMA_INVALID: %w", err)
			}
		} else if !os.IsNotExist(readErr) {
			return ChangeAnalysis{}, Certificate{}, fmt.Errorf("CHANGE_SET_SNAPSHOT_SCHEMA_READ_FAILED: %w", readErr)
		}
		...
		if err := compareDraftChangeSet153(meta.ChangedFiles, live.Files); err != nil {
