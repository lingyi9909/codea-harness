...
func TestFixFlowRequiresRuntimeApplyEvidence(t *testing.T) {
	for _, parts := range [][]string{{"agents", "fix-agent.md"}, {"skills", "fix-bug", "SKILL.md"}} {
		...
		for _, want := range []string{"apply-request.schema.json", "codea-dcep-tools.exe seal-apply --input", "sealed-plans", "codea-dcep-tools.exe apply --input", "evidence/apply", "diffSha256", "baseSha256", "审批前"} {
...
func TestTestGenerationRequiresRuntimeApplyEvidence(t *testing.T) {
	for _, parts := range [][]string{{"agents", "integration-test-agent.md"}, {"skills", "generate-integration-tests", "SKILL.md"}, {"skills", "design-integration-tests", "SKILL.md"}} {
		text := readHarnessContract(t, parts...)
		for _, want := range []string{"unifiedDiff", "diffSha256", "baseSha256", "codea-dcep-tools.exe seal-apply --input", "sealed-plans", "codea-dcep-tools.exe apply --input", "planType=TEST", "审批前"} {
			if !strings.Contains(text, want) {
				t.Fatalf("%s missing %q", filepath.Join(parts...), want)
			}
		}
...
func TestOrchestratorDefinesRuntimeApplySafetyGate(t *testing.T) {
	...
	for _, want := range []string{"Runtime Apply Safety Gate", "apply-request.schema.json", "apply-result.schema.json", "codea-dcep-tools.exe seal-apply --input", "sealed-plans", "APPROVAL_IDENTITY_MISMATCH", "BASE_CHANGED", "PLAN_ALREADY_APPLIED", "rollbackPerformed", "direct host write", ".git/**", ".code-harness/**"} {
		if !strings.Contains(text, want) {
