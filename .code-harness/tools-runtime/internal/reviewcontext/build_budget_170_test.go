...
func Test170SourceByteBudgetBlocksAdditionalContext(t *testing.T) {
	...
	if len(got.Relations) != 0 {
		t.Fatalf("oversize relation escaped budget: %+v", got.Relations)
	}
	if got.Usage.SourceBytes != 0 {
		t.Fatalf("rejected source counted as consumed: %+v", got.Usage)
	}
...
func Test170BudgetRejectedEdgeDoesNotExpandQueue(t *testing.T) {
	...
	budget := DefaultBudget170()
	budget.MaxSourceBytes = 32
	if _, err := Build170(context.Background(), BuildInput170{RunID: "review-case", Phase: "DISCOVERY", Seeds: []nav.ReviewRef170{seed}, Budget: budget}, resolver); err != nil {
		t.Fatal(err)
	}
	for _, call := range resolver.methodCalls {
		if call == targetKey {
			t.Fatalf("budget-rejected target was still expanded: calls=%v", resolver.methodCalls)
		}
	}
}
...
func Test170RecursionBoundaryIsExplicit(t *testing.T) {
	...
	for _, issue := range got.Issues {
		if issue.Code == "RECURSION_BOUNDARY" {
			found = true
		}
	}
	if !found {
		t.Fatalf("missing RECURSION_BOUNDARY: issues=%+v relations=%+v", got.Issues, got.Relations)
	}
}
