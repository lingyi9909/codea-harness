...
// contract resolution. The private verified bit prevents callers outside this
// package from turning an arbitrary sibling directory into provider authority;
// product callers obtain roots through ProviderRootsFromVerification170.
...
// ProviderRootsFromVerification170 is the only product-facing constructor for
// dependency provider roots. A root participates only after the existing Maven
// workspace identity gate has returned VERIFIED with a confirmed root.
func ProviderRootsFromVerification170(results []workspace.VerificationResult) []ProviderRoot170 {
	...
	for _, result := range results {
		...
		id := strings.TrimSpace(result.DependencyID)
		root := strings.TrimSpace(result.ConfirmedRoot)
		if id == "" || root == "" {
			continue
		}
		key := strings.ToLower(id) + "\x00" + strings.ToLower(filepath.Clean(root))
		if seen[key] {
			continue
		}
		seen[key] = true
		out = append(out, ProviderRoot170{Workspace: id, Root: filepath.Clean(root), verified: true})
	}
...
// the current repository and already-verified workspace dependencies. It does
// not query a registry, execute Maven, invoke RPC, or claim runtime routing.
func ResolveDubbo170(ctx context.Context, currentRoot string, consumer nav.ReviewRef170, providers []ProviderRoot170) ([]nav.Relation170, []nav.Issue170, error) {
	...
	packageName := dubboPackage170(data)
	imports := dubboImports170(data)
	classStart, classEnd, ok := dubboOwnerRange170(data, simpleJavaName170(consumer.OwnerFQCN))
	if !ok {
	...
	methods := dubboMethods170(data, classStart, classEnd, consumer.Name, packageName, imports)
	caller, ok := dubboSelectMethod170(methods, consumer.ParameterTypes)
	if !ok {
	...
	props := dubboLocalProperties170(root)
	references := dubboReferences170(data, classStart, classEnd, packageName, imports, props)
	call, reference, remoteMethod, argTypes, ok := dubboRemoteCall170(data, caller, references, packageName, imports)
	if !ok {
	...
	callEvidence := dubboRange170(data, consumer, call[0], call[1])
	...
	if !reference.Resolved {
		...
		issue := nav.Issue170{Code: "DUBBO_CONFIG_UNRESOLVED", At: callEvidence, Detail: reason}
		return []nav.Relation170{relation}, []nav.Issue170{issue}, nil
	}
	...
	if len(matching) == 1 {
		...
		relation := nav.Relation170{
			...
			Targets: []nav.ReviewRef170{target}, Evidence: append(evidence, targetEvidence...),
...
func dubboReferences170(data []byte, start, end int, packageName string, imports map[string]string, props map[string]string) []dubboReference170 {
	...
	for _, annotation := range annotations {
		...
		groupRaw := attrs["group"]
		versionRaw := attrs["version"]
		...
		out = append(out, dubboReference170{
			InterfaceFQCN: dubboResolveType170(string(match[1]), packageName, imports),
			Field: string(match[2]), GroupRaw: groupRaw, Group: group,
			VersionRaw: versionRaw, Version: version, Resolved: groupOK && versionOK,
			At: annotation,
		})
	}
	return out
...
func dubboRemoteCall170(data []byte, caller dubboMethod170, refs []dubboReference170, packageName string, imports map[string]string) ([2]int, dubboReference170, string, []string, bool) {
	var none [2]int
	...
	for _, ref := range refs {
		...
		args := body[loc[4]:loc[5]]
		argTypes := []string{}
		for _, arg := range dubboSplitComma170(args) {
			argTypes = append(argTypes, dubboInferArgumentType170(strings.TrimSpace(arg), paramTypes, packageName, imports))
		}
...
func dubboProviderMethods170(ctx context.Context, root dubboRoot170, interfaceFQCN, method string) ([]dubboProviderMethod170, error) {
	...
	for _, path := range paths {
		...
		imports := dubboImports170(data)
		annotations := dubboAnnotations170(data, 0, len(data), imports, dubboServiceAnnotations170)
		for _, annotation := range annotations {
			...
			methods := dubboMethods170(data, open+1, close, method, pkg, imports)
			...
			for _, candidate := range methods {
				out = append(out, dubboProviderMethod170{
					...
					InterfaceFQCN: providerInterface, Name: method, ParameterTypes: candidate.ParameterTypes,
					GroupRaw: groupRaw, Group: group, VersionRaw: versionRaw, Version: version,
					ConfigResolved: groupOK && versionOK, Start: candidate.Start, End: candidate.End, Data: data,
				})
			}
		}
	}
...
func dubboMethods170(data []byte, start, end int, name, pkg string, imports map[string]string) []dubboMethod170 {
	...
	for _, match := range matches {
		...
		if data[terminal] == ';' {
			...
			method.BodyStart = terminal + 1
			method.BodyEnd = close
			method.End = close + 1
		}
...
func dubboInferArgumentType170(arg string, params map[string]string, pkg string, imports map[string]string) string {
	...
	if regexp.MustCompile(`^-?[0-9]+[lL]$`).MatchString(arg) {
		return "long"
	}
...
func dubboTypeEqual170(left, right string) bool {
	...
	return simpleJavaName170(left) == simpleJavaName170(right)
}
...
func dubboRange170(data []byte, ref nav.ReviewRef170, start, end int) nav.SourceRange170 {
	...
	if end <= start { end = start + 1; if end > len(data) { start = len(data)-1; end = len(data) } }
...
func simpleJavaName170(value string) string {
	value = strings.TrimSpace(value)
