...
func (f *fakeResolver170) Spring(context.Context, nav.ReviewRef170) ([]nav.Relation170, []nav.Issue170, error) {
	return []nav.Relation170{}, []nav.Issue170{}, nil
}
...
func Test170ParallelCallsStaySeparate(t *testing.T) {
	...
	for _, rel := range got.Relations {
		if rel.From.OwnerFQCN != "demo.A" {
			t.Fatalf("parallel call was serialized through sibling target: %+v", rel)
		}
	}
}
...
func Test170BudgetReportsBlocked(t *testing.T) {
	...
	for _, c := range got.Checks {
		...
		reasons[c.RuleID] = c.Reasons
	}
	if status["LOCAL"] != "READY" {
		t.Fatalf("local=%s checks=%+v", status["LOCAL"], got.Checks)
	}
	if status["REMOTE"] != "BLOCKED" {
		t.Fatalf("remote=%s checks=%+v", status["REMOTE"], got.Checks)
	}
	if !strings.Contains(strings.Join(reasons["REMOTE"], " "), "CONTEXT_BUDGET_EXCEEDED") {
		t.Fatalf("remote reasons=%v", reasons["REMOTE"])
	}
}
...
func Test170UntrackedSeedIncluded(t *testing.T) {
	...
	if len(got.Relations) == 0 || len(resolver.methodCalls) == 0 {
		t.Fatalf("untracked seed was filtered: %+v", got)
	}
}
...
func Test170DeletedMethodBeforeContext(t *testing.T) {
	...
	if len(got.Relations) != 0 || len(resolver.methodCalls) != 0 {
		t.Fatalf("BASE-only deleted method became CURRENT context: %+v calls=%v", got.Relations, resolver.methodCalls)
	}
}

func Test170ContextCannotExpandScope(t *testing.T) {
	seed := ref170("src/main/java/demo/A.java", "demo.A", "submit")
	dep := ref170("src/main/java/shared/Shared.java", "shared.Shared", "check")
	dep.Workspace = "shared-verified"
	...
	if len(got.Relations) != 1 || got.Relations[0].Targets[0].Workspace != "shared-verified" {
		t.Fatalf("dependency context missing: %+v", got.Relations)
	}
	if len(input.Seeds) != 1 || input.Seeds[0].Workspace != "current" {
		t.Fatalf("Build170 mutated authoritative selection input: %+v", input.Seeds)
	}
}
...
func Test170ForgedRelationRejected(t *testing.T) {
	...
	input := BuildInput170{RunID: "review-case", Phase: "DISCOVERY", Seeds: []nav.ReviewRef170{seed}, Budget: DefaultBudget170()}
	ctx, err := Build170(context.Background(), input, resolver)
	if err != nil {
		t.Fatal(err)
	}
...
func Test170BuildRejectsResolverFailure(t *testing.T) {
	...
	if _, err := Build170(context.Background(), BuildInput170{RunID: "review-case", Phase: "DISCOVERY", Seeds: []nav.ReviewRef170{seed}, Budget: DefaultBudget170()}, resolver); err == nil {
...
func (errorResolver170) Method(context.Context, nav.ReviewRef170) (nav.MethodFacts170, error) {
	return nav.MethodFacts170{}, errors.New("boom")
}
func (errorResolver170) Callers(context.Context, nav.ReviewRef170) ([]nav.Relation170, error) {
	return nil, errors.New("boom")
}
func (errorResolver170) Mapper(context.Context, nav.SourceRange170) ([]nav.Relation170, []nav.Issue170, error) {
	return nil, nil, errors.New("boom")
}
func (errorResolver170) Dubbo(context.Context, nav.ReviewRef170) ([]nav.Relation170, []nav.Issue170, error) {
	return nil, nil, errors.New("boom")
}

func (errorResolver170) Spring(context.Context, nav.ReviewRef170) ([]nav.Relation170, []nav.Issue170, error) {
	return nil, nil, errors.New("boom")
}
