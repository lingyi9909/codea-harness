...
type task6FixtureFile160 struct { Path string `json:"path"`; Workspace string `json:"workspace"`; Changed bool `json:"changed"`; BeforeContent string `json:"beforeContent"`; Content string `json:"content"` }
type task6FixtureExpected160 struct { RuleDispatched bool `json:"ruleDispatched"` }
type task6Fixture160 struct { ID string `json:"id"`; Class string `json:"class"`; RuleID string `json:"ruleId"`; Files []task6FixtureFile160 `json:"files"`; Expected task6FixtureExpected160 `json:"expected"` }
...
func TestTask6ProblemFixturesEncodeRuleGroundTruthInSource(t *testing.T) {
	...
	checks := map[string]func(string,string) bool{
		"03-mybatis-dollar-bind": func(before, after string) bool { return strings.Contains(after, "${sort}") && strings.Contains(after, "user-controlled input") && strings.Contains(after, "String sort") },
		"05-tx-self-invocation": func(before, after string) bool { return strings.Contains(after, "@Transactional") && strings.Contains(after, "inner();") && strings.Contains(before, "proxy.inner();") },
		"11-test-validity": func(before, after string) bool { return strings.Contains(before, "assertEquals(") && !strings.Contains(after, "assertEquals(") },
	}
...
func TestTask6TestValidityPathRoleIsRuntimeOwned(t *testing.T) {
	...
	for _, want := range []string{"RULE_DISPATCH_PATH_ROLE_INVALID", `src/test/`, `role Test requires src/test path`, `changedRoles["Test"]`} {
		if !strings.Contains(dispatch, want) { t.Fatalf("Runtime Test path-role invariant missing %q", want) }
	}
}
...
func TestTask6DeterminismComparesCanonicalArtifacts(t *testing.T) {
	...
	for _, want := range []string{"reviewunit.CanonicalBytes(left.Units)", "reviewrules.CanonicalBytes(left.Dispatch)", "canonicalCertifiedSet160(left.Set,true)", "left.Units.SHA256", "left.Dispatch.SHA256", "left.Set.SHA256"} {
		if !strings.Contains(text, want) { t.Fatalf("determinism benchmark missing %s", want) }
	}
}
...
func TestTask6BenchmarkUsesFormalPrecisionAndSemanticDedupIdentity(t *testing.T) {
	...
	if !strings.Contains(text, "semanticIdentity160(v)") { t.Fatal("DuplicateRate must reuse formal semanticIdentity160") }
	dedup := string(task6Read160(t, "dedup.go"))
	if !strings.Contains(dedup, "v.EvidenceDigest") { t.Fatal("formal semantic dedup identity must include evidenceDigest") }
}
