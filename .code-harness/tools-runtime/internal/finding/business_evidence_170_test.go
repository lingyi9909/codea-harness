...
func Test170BusinessRuleEvidenceRequiresBoundKnowledgeAndCodeEvidence(t *testing.T) {
	...
	manifest := knowledge.Manifest170{
		...
		Sources: []knowledge.SourceRecord170{{SourceID:"order-rules", Root:"PROJECT", Path:"docs/order-rules.md", Kind:"RULES", Required:true, SHA256:strings.Repeat("2",64), Status:"READY", Reasons:[]string{}, Rule:&knowledge.Rule170{RuleID:"ORDER-001", ProjectID:"order-service", Status:"ACTIVE", Version:"2026-09", Owner:"risk", Source:"docs/order-rules.md", ApprovalRef:"APP-1", AppliesTo:knowledge.AppliesTo170{Paths:[]string{"src/main/"}}, Supersedes:[]string{}, Exceptions:[]string{}}}},
...
func Test170KnowledgeDocumentCannotBecomeCodeAnchor(t *testing.T) {
	ctx := verifyContext160(t)
	ctx.knowledge170 = &knowledge.Manifest170{RunID:"run-task3", ProjectID:"order-service", Status:"READY", BindingSHA256:strings.Repeat("1",64), Sources:[]knowledge.SourceRecord170{{SourceID:"order-rules", Root:"PROJECT", Path:servicePath160, Kind:"RULES", Required:true, SHA256:strings.Repeat("2",64), Status:"READY", Rule:&knowledge.Rule170{RuleID:"ORDER-001", ProjectID:"order-service", Status:"ACTIVE", Version:"1", Owner:"risk", Source:servicePath160, ApprovalRef:"A", AppliesTo:knowledge.AppliesTo170{Paths:[]string{"./"}}}}}
	p := baseProposal160()
