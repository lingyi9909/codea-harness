...
func Test170BusinessReviewCheckRequiresActuallyReadSource(t *testing.T) {
	...
	for _, d := range ctx.dispatch.Dispatches {
		if d.ReviewUnitID != "RU-TASK3" { continue }
	...
	for i := range checks { if checks[i].RuleID == "BUSINESS:rules:R-1" { checks[i].SourceIDs = []string{"rules"} } }
	summary, err := ValidateCheckResults170(ctx, checks)
	if err != nil { t.Fatal(err) }
	if summary.Status != "COMPLETE" { t.Fatalf("all dispatched READY checks completed should be complete, got %#v", summary) }

	for i := range checks { if checks[i].RuleID == "BUSINESS:rules:R-1" { checks[i].Status="INCOMPLETE"; checks[i].Reason="rule conflict requires manual review" } }
	summary, err = ValidateCheckResults170(ctx, checks)
	if err != nil { t.Fatal(err) }
	if summary.Status != "PARTIAL" { t.Fatalf("explicit reviewer INCOMPLETE must produce PARTIAL, got %#v", summary) }
}
