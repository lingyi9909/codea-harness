...
func TestCertifyProducesOneFindingForSemanticDuplicates(t *testing.T) {
	...
	if err != nil { t.Fatalf("certify semantic duplicates: %v", err) }
	if len(rejections) != 0 { t.Fatalf("valid semantic duplicates must not be rejected: %+v", rejections) }
	if len(set.Findings) != 1 { t.Fatalf("semantic duplicates must collapse to one Certified Finding, got %d", len(set.Findings)) }
}
...
func TestCertifyRejectsUnverifiedProposalWithoutBlockingOtherValidProposal(t *testing.T) {
	...
	if err != nil { t.Fatalf("single proposal rejection must not abort certification: %v", err) }
	if len(set.Findings) != 1 || len(rejections) != 1 { t.Fatalf("expected one Certified Finding and one rejection, findings=%d rejections=%+v", len(set.Findings), rejections) }
	if rejections[0].ProposalID != "P-INVALID" { t.Fatalf("unexpected rejection identity: %+v", rejections[0]) }
}
