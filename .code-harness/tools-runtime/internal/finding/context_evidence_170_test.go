...
func Test170ContextRelationEvidenceAcceptsVerifiedCurrentRelation(t *testing.T) {
	...
	source := nav.ReviewRef170{Workspace: "current", Path: servicePath160, Side: "CURRENT", Kind: "METHOD", OwnerFQCN: "com.acme.OrderServiceImpl", Name: "approve", ParameterTypes: []string{}}
	ctx.reviewContext170 = &reviewcontext.Context170{
		RunID: "run-task3", Phase: "RULES",
		Relations: []nav.Relation170{{
			ID: "REL-170-1", Kind: "JAVA_CALL", Resolution: "EXACT", From: source,
			Targets:     []nav.ReviewRef170{target},
			Evidence:    []nav.SourceRange170{{Ref: source, StartLine: 3, EndLine: 3, StartColumn: 3, EndColumn: 20}},
			Assumptions: []string{},
		}},
	...
	attacks := []struct {
		...
		{"target tamper", func(p *Proposal) {
			p.EvidenceRefs[0].Value = strings.ReplaceAll(targetKey, "OrderController", "OtherController")
		}},
	}
...
func Test170ContextRelationCannotBorrowAnotherUnitCheck(t *testing.T) {
	...
	ctx.reviewContext170 = &reviewcontext.Context170{
		RunID: "run-task3", Phase: "RULES",
		Relations: []nav.Relation170{{ID: "REL-OTHER", Kind: "JAVA_CALL", Resolution: "EXACT", From: source, Targets: []nav.ReviewRef170{target}, Evidence: []nav.SourceRange170{{Ref: source, StartLine: 3, EndLine: 3, StartColumn: 3, EndColumn: 20}}, Assumptions: []string{}}},
