...
func Test170KnowledgeDocumentAndByteBudgetsFailClosed(t *testing.T) {
	t.Run("document-count", func(t *testing.T) {
		...
		for i := 0; i < 21; i++ {
			...
			if err := os.WriteFile(filepath.Join(repo, "docs", name), []byte("x"), 0o644); err != nil {
				t.Fatal(err)
			}
		...
		if len(result.Documents) != 20 || result.Manifest.Status != "PARTIAL" {
			t.Fatalf("document cap not enforced: docs=%d manifest=%+v", len(result.Documents), result.Manifest)
		}
		blocked := 0
		for _, source := range result.Manifest.Sources {
			if source.Status == "BLOCKED" && strings.Contains(strings.Join(source.Reasons, " "), "BUDGET_DOCUMENTS") {
				blocked++
			}
		}
		if blocked != 1 {
			t.Fatalf("expected exactly one document-budget block, got %d", blocked)
		}
	})
	...
	t.Run("byte-count", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(repo, "docs", "a.md"), []byte(strings.Repeat("a", 140*1024)), 0o644); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(repo, "docs", "b.md"), []byte(strings.Repeat("b", 140*1024)), 0o644); err != nil {
			t.Fatal(err)
		}
		...
		if len(result.Documents) != 1 || result.Manifest.Status != "PARTIAL" {
			t.Fatalf("byte cap not enforced: docs=%d manifest=%+v", len(result.Documents), result.Manifest)
		}
		if result.Manifest.Sources[1].Status != "BLOCKED" || !strings.Contains(strings.Join(result.Manifest.Sources[1].Reasons, " "), "BUDGET_BYTES") {
			t.Fatalf("second source must be byte-budget blocked: %+v", result.Manifest.Sources)
		}
	})
}
...
func Test170KnowledgeDraftRetiredAndMetadataNeverBecomeActiveRules(t *testing.T) {
	for _, status := range []string{"DRAFT", "RETIRED"} {
		t.Run(status, func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(repo, "rules", "rule.md"), []byte(raw), 0o644); err != nil {
				t.Fatal(err)
			}
			...
			if result.Manifest.Status != "READY" || len(result.Manifest.Checks) != 0 || len(result.Manifest.Sources) != 1 || result.Manifest.Sources[0].Status != "NOT_APPLICABLE" {
				t.Fatalf("inactive rule became effective: %+v", result.Manifest)
			}
		})
	}
...
body
`)
	if _, _, err := ParseRule170(missingApproval); err == nil {
		t.Fatal("ACTIVE rule without approvalRef accepted")
	}
...
body
`)
	if rule, _, err := ParseRule170(metadataOnly); err != nil || rule.Source == "" || rule.ApprovalRef == "" {
		t.Fatalf("source/approvalRef must remain opaque metadata, rule=%+v err=%v", rule, err)
	}
}
