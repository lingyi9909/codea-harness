...
func Test170KnowledgeParseBindingStrict(t *testing.T) {
	...
	if binding.ProjectID != "order-service" || len(binding.Sources) != 1 || len(digest) != 64 {
		t.Fatalf("unexpected binding: %+v digest=%q", binding, digest)
	}
...
func Test170KnowledgeParseRuleAndApplicability(t *testing.T) {
	...
	if rule.RuleID != "ORDER-STATE-001" || strings.TrimSpace(body) == "" {
		t.Fatalf("unexpected rule/body: %+v %q", rule, body)
	}
	if !Applies170(rule, Unit170{ID: "u1", Paths: []string{"src/main/java/example/order/OrderService.java"}}) {
		t.Fatal("directory prefix did not apply")
	}
	if Applies170(rule, Unit170{ID: "u2", Paths: []string{"src/main/java/example/payment/PaymentService.java"}}) {
		t.Fatal("rule leaked into sibling module")
	}

	all, _, err := ParseRule170(activeRule170("ALL-001", "*", "./"))
	if err != nil || !Applies170(all, Unit170{ID: "u3", Paths: []string{"any/project/path.txt"}}) {
		t.Fatalf("./ selector must apply to all project units: %v", err)
	}
...
`)
	...
	if !Applies170(exact, Unit170{ID: "s", EntryPoints: []string{"com.acme.RiskService#check(java.lang.String)"}}) {
		t.Fatal("exact entry point did not apply")
	}
	if Applies170(exact, Unit170{ID: "l", EntryPoints: []string{"com.acme.RiskService#check(java.lang.Long)"}}) {
		t.Fatal("signature selector matched different overload")
	}
	if Applies170(exact, Unit170{ID: "b", EntryPoints: []string{"com.acme.RiskService"}}) {
		t.Fatal("bare class matched exact method selector")
	}
	...
	for _, input := range bad {
		if _, _, err := ParseRule170(input); err == nil {
			t.Fatalf("invalid rule accepted: %q", input)
		}
	}
}
...
func Test170KnowledgeLoadProjectAndTeamSources(t *testing.T) {
	repo := t.TempDir()
	team := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, "docs", "business"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(team, "refs"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(repo, "docs", "business", "order.md"), activeRule170("ORDER-STATE-001", "order-service", "src/main/java/example/order/"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if result.Manifest.Status != "READY" || len(result.Documents) != 2 {
		t.Fatalf("unexpected result: %+v docs=%d", result.Manifest, len(result.Documents))
	}
	if len(result.Manifest.Checks) != 1 || result.Manifest.Checks[0].ReviewUnitID != "unit-order" || result.Manifest.Checks[0].RuleKey != "BUSINESS:order-rule:ORDER-STATE-001" || result.Manifest.Checks[0].Status != "READY" {
		t.Fatalf("unexpected checks: %+v", result.Manifest.Checks)
	}
	for _, source := range result.Manifest.Sources {
		if source.SHA256 == "" || len(source.SHA256) != 64 {
			t.Fatalf("source digest missing: %+v", source)
		}
	}
}
...
func Test170KnowledgeRequiredUnknownApplicabilityBlocksSelectedUnits(t *testing.T) {
	...
	if result.Manifest.Status != "PARTIAL" || len(result.Manifest.Checks) != 2 {
		t.Fatalf("required unreadable rule must conservatively block selected units: %+v", result.Manifest)
	}
	for _, check := range result.Manifest.Checks {
		if check.Status != "BLOCKED" || !strings.Contains(strings.Join(check.Reasons, " "), "SOURCE") {
...
func Test170KnowledgeStateDuplicateAndProjectRulesFailClosed(t *testing.T) {
	repo := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, "rules"), 0o755); err != nil {
		t.Fatal(err)
	}
	write := func(name string, content []byte) {
		t.Helper()
		if err := os.WriteFile(filepath.Join(repo, "rules", name), content, 0o644); err != nil {
			t.Fatal(err)
		}
	}
	...
	if result.Manifest.Status != "PARTIAL" || !strings.Contains(strings.Join(result.Manifest.Issues, " "), "DUPLICATE_RULE_ID") {
		t.Fatalf("duplicate formal ruleId must block: %+v", result.Manifest)
	}
	...
	if result.Manifest.Status != "PARTIAL" || len(result.Manifest.Checks) != 1 || result.Manifest.Checks[0].Status != "BLOCKED" {
		t.Fatalf("project mismatch became effective: %+v", result.Manifest)
	}
}
...
func Test170KnowledgeBudgetCountsUniqueFilesAndCapsExperience(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(repo, "docs", "shared.md"), refContent, 0o644); err != nil {
		t.Fatal(err)
	}
	sources := []Source170{
	...
	if len(result.Documents) != 7 {
		t.Fatalf("same final file must be read once per content but returned per declared source until budget; want shared x2 + max 5 experiences =7, got %d", len(result.Documents))
	}
	blockedExperience := 0
	for _, source := range result.Manifest.Sources {
		if source.Kind == "EXPERIENCE" && source.Status == "BLOCKED" {
			blockedExperience++
		}
	}
	if blockedExperience != 2 {
		t.Fatalf("experience cap must block exactly two sources, got %d: %+v", blockedExperience, result.Manifest.Sources)
	}
}
...
func Test170KnowledgeRejectsNonUTF8NonRegularAndEscapingSymlink(t *testing.T) {
	repo := t.TempDir()
	outside := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, "docs", "dir.md"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(repo, "docs", "binary.md"), []byte{0xff, 0xfe, 0xfd}, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Symlink(outsideFile, link); err != nil {
		t.Fatalf("test requires symlink support: %v", err)
	}
	...
	if result.Manifest.Status != "PARTIAL" {
		t.Fatalf("invalid local materials must be partial: %+v", result.Manifest)
	}
	joined := strings.Join(result.Manifest.Issues, " ")
	for _, want := range []string{"NOT_REGULAR", "UTF8", "OUTSIDE_ROOT"} {
		if !strings.Contains(joined, want) {
			t.Fatalf("missing %s issue: %s", want, joined)
		}
	}
}
...
func Test170KnowledgeVerifyDetectsBindingAndSourceChanges(t *testing.T) {
	...
	if err := os.WriteFile(path, activeRule170("ORDER-001", "order-service", "./"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := Verify170(input, loaded.Manifest); err != nil {
		t.Fatalf("fresh knowledge rejected: %v", err)
	}
	if err := os.WriteFile(path, activeRule170("ORDER-001", "order-service", "src/main/java/changed/"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := Verify170(input, loaded.Manifest); err == nil || !strings.Contains(err.Error(), "KNOWLEDGE_SOURCE_CHANGED") {
		t.Fatalf("source mutation must be rejected, got %v", err)
	}
	input.BindingSHA256 = strings.Repeat("2", 64)
	if err := Verify170(input, loaded.Manifest); err == nil || !strings.Contains(err.Error(), "KNOWLEDGE_BINDING_CHANGED") {
		t.Fatalf("binding mutation must be rejected, got %v", err)
	}
}
...
func Test170KnowledgeBindingDigestMatchesRawBytes(t *testing.T) {
	...
	if digest != want {
		t.Fatalf("binding digest mismatch: got=%s want=%s", digest, want)
	}
}
