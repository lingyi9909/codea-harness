...
func Load170(in LoadInput170) (LoadResult170, error) {
	...
	for i, source := range in.Binding.Sources { ordered[i] = indexedSource{index: i, source: source} }
	...
	for _, item := range ordered {
		...
		if rootErr != nil {
			...
			if source.Kind == "RULES" { appendUnknownChecks170(&result.Manifest, in.Units, source.ID, record.Reasons) }
			continue
		}
		...
		if source.Kind == "EXPERIENCE" { experiencePaths[resolved] = true }
		...
		if source.Kind == "RULES" {
			...
			if rule.Status != "ACTIVE" {
				...
				if !applicable { record.Status = "NOT_APPLICABLE" }
			}
		}
		records[item.index] = record
	...
	if result.Manifest.Status == "READY" {
		for _, record := range records {
			if record.Required && record.Status == "BLOCKED" { result.Manifest.Status = "PARTIAL"; break }
		}
	}
	for _, check := range result.Manifest.Checks { if check.Status == "BLOCKED" { result.Manifest.Status = "PARTIAL"; break } }
...
func sourcePriority170(source Source170) int {
	if source.Kind == "RULES" { return 0 }
	if source.Required { return 1 }
	if source.Kind == "REFERENCE" { return 2 }
	return 3
}

func sourceRoot170(repoRoot, teamRoot, root string) (string, error) {
	if root == "PROJECT" { return filepath.EvalSymlinks(repoRoot) }
	if root != "TEAM" { return "", fmt.Errorf("unknown root %q", root) }
	value := strings.TrimSpace(teamRoot)
	if value == "" { return "", fmt.Errorf("teamRoot is empty") }
	if !filepath.IsAbs(value) { value = filepath.Join(repoRoot, value) }
...
func resolveSourcePath170(root, sourcePath string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) { return "", fmt.Errorf("KNOWLEDGE_SOURCE_MISSING: %s", sourcePath) }
		return "", fmt.Errorf("KNOWLEDGE_SOURCE_RESOLVE_FAILED: %s: %w", sourcePath, err)
	}
	rel, err := filepath.Rel(root, resolved)
	if err != nil { return "", fmt.Errorf("KNOWLEDGE_SOURCE_OUTSIDE_ROOT: %s", sourcePath) }
	if rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) || filepath.IsAbs(rel) {
		return "", fmt.Errorf("KNOWLEDGE_SOURCE_OUTSIDE_ROOT: %s", sourcePath)
	}
	info, err := os.Stat(resolved)
	if err != nil { return "", fmt.Errorf("KNOWLEDGE_SOURCE_STAT_FAILED: %s: %w", sourcePath, err) }
	if !info.Mode().IsRegular() { return "", fmt.Errorf("KNOWLEDGE_SOURCE_NOT_REGULAR: %s", sourcePath) }
	return resolved, nil
}

func readStableUTF8File170(path string) ([]byte, error) {
	before, err := os.Stat(path)
	if err != nil { return nil, fmt.Errorf("KNOWLEDGE_SOURCE_READ_FAILED: %w", err) }
	first, err := os.ReadFile(path)
	if err != nil { return nil, fmt.Errorf("KNOWLEDGE_SOURCE_READ_FAILED: %w", err) }
	if !utf8.Valid(first) { return nil, fmt.Errorf("KNOWLEDGE_SOURCE_UTF8_INVALID: %s", filepath.Base(path)) }
	after, err := os.Stat(path)
	if err != nil { return nil, fmt.Errorf("KNOWLEDGE_SOURCE_CHANGED: %w", err) }
...
func blockSource170(manifest *Manifest170, record *SourceRecord170, required bool, reason string) {
	...
	if required { manifest.Status = "PARTIAL" }
}

func appendUnknownChecks170(manifest *Manifest170, units []Unit170, sourceID string, reasons []string) {
	if len(reasons) == 0 { reasons = []string{"KNOWLEDGE_SOURCE_UNAVAILABLE"} }
...
func appendRuleChecks170(manifest *Manifest170, units []Unit170, sourceID string, rule Rule170, status string, reasons []string) bool {
	matched := false
	for _, unit := range units {
		if !Applies170(rule, unit) { continue }
		matched = true
...
func sortManifest170(manifest *Manifest170) {
	...
	sort.Slice(manifest.Checks, func(i, j int) bool {
		if manifest.Checks[i].ReviewUnitID != manifest.Checks[j].ReviewUnitID { return manifest.Checks[i].ReviewUnitID < manifest.Checks[j].ReviewUnitID }
		return manifest.Checks[i].RuleKey < manifest.Checks[j].RuleKey
	})
	manifest.Issues = uniqueStrings170(manifest.Issues)
	for i := range manifest.Sources { manifest.Sources[i].Reasons = uniqueStrings170(manifest.Sources[i].Reasons) }
	for i := range manifest.Checks { manifest.Checks[i].Reasons = uniqueStrings170(manifest.Checks[i].Reasons) }
}

func uniqueStrings170(in []string) []string {
	seen := map[string]bool{}
	out := make([]string, 0, len(in))
	for _, value := range in { value = strings.TrimSpace(value); if value != "" && !seen[value] { seen[value] = true; out = append(out, value) } }
	sort.Strings(out)
	return out
}
