...
func Test170KnowledgeBusinessDispatchBindsManifestDigest(t *testing.T) {
	...
	if manifest.KnowledgeSHA256 != knowledgeSHA {
		t.Fatalf("knowledge digest mismatch got=%s want=%s", manifest.KnowledgeSHA256, knowledgeSHA)
	}
	if manifest.RuleCatalogSHA256 == catalogSHA || len(manifest.RuleCatalogSHA256) != 64 {
		t.Fatalf("effective catalog must bind built-in + knowledge digest: %s", manifest.RuleCatalogSHA256)
	}

	var business *Dispatch
	for i := range manifest.Dispatches {
		if manifest.Dispatches[i].RuleID == "BUSINESS:order-rules:ORDER-STATE-001" {
			business = &manifest.Dispatches[i]
			break
		}
	}
	if business == nil {
		t.Fatalf("READY business check not dispatched: %+v", manifest.Dispatches)
	}
	if business.Kind != KindAgent || business.RuleVersion != 1 || business.SeverityDefault != "medium" {
		t.Fatalf("unexpected BUSINESS dispatch: %+v", *business)
	}
	if !reflect.DeepEqual(business.RequiredEvidence, []string{"BUSINESS_RULE", "CHANGED_RANGE"}) {
		t.Fatalf("business evidence=%v", business.RequiredEvidence)
	}
	if !reflect.DeepEqual(business.DispatchReason, []string{"BUSINESS_RULE:order-rules:ORDER-STATE-001"}) {
		t.Fatalf("business reason=%v", business.DispatchReason)
	}
}
...
func Test170KnowledgeBlockedChecksDoNotBecomeEffectiveDispatch(t *testing.T) {
	...
	for _, d := range manifest.Dispatches {
		if strings.HasPrefix(d.RuleID, "BUSINESS:") {
			t.Fatalf("blocked knowledge became effective dispatch: %+v", d)
		}
	}
}
...
func Test170KnowledgeLegacyDispatchRemainsKnowledgeFree(t *testing.T) {
	...
	if legacy.KnowledgeSHA256 != "" || legacy.RuleCatalogSHA256 != catalogSHA {
		t.Fatalf("legacy dispatch semantics changed: %+v", legacy)
	}
}
