...
func Test170PartialReviewContextPreservesCertifiedFindingAndRequiresManualAction(t *testing.T) {
	...
	if got.Result != ResultManualActionRequired { t.Fatalf("PARTIAL must require manual action, got %s", got.Result) }
	if got.Coverage.Status != "PARTIAL" { t.Fatalf("PARTIAL summary must force partial coverage, got %s", got.Coverage.Status) }
	if len(got.Findings) != 1 || got.Findings[0].ID != "CF-1" { t.Fatalf("certified finding must be preserved during partial review: %#v", got.Findings) }
	if len(got.Coverage.Unresolved) == 0 { t.Fatal("blocked check must be visible as an incomplete-review gap") }
}
...
func Test170PartialZeroFindingCannotBecomePassed(t *testing.T) {
	...
	got, err := finalizeCertifiedRequest170(req, set)
	if err != nil { t.Fatal(err) }
	if got.Result == ResultPassed { t.Fatal("zero findings with blocked checks must not pass") }
}
...
func Test170CompleteZeroFindingCanPass(t *testing.T) {
	...
	got, err := finalizeCertifiedRequest170(req, set)
	if err != nil { t.Fatal(err) }
	if got.Result != ResultPassed || got.Coverage.Status != "COMPLETE" { t.Fatalf("complete zero-finding review should pass, got result=%s coverage=%s", got.Result, got.Coverage.Status) }
}
