...
func TestFindImplementationsSupportsSuperclassAndInterface(t *testing.T){
	r:=&fakeRunner{out:[]byte{}}
	n:=nav.Navigator{RepoRoot:`C:\\repo`,AstGrepPath:`C:\\repo\\.code-harness\\bin\\ast-grep.exe`,Runner:r}
	_,err:=n.FindImplementations(context.Background(),"OrderService","src/main/java"); if err!=nil{t.Fatal(err)}
	want:=[]string{"--lang","java","--json=stream","--pattern","public class $C extends $SUPER implements OrderService { $$$BODY }","src/main/java"}
	found:=false
	for _,call:=range r.calls{if reflect.DeepEqual(call,want){found=true}}
	if !found{t.Fatalf("extends + implements pattern missing: %v",r.calls)}
}
