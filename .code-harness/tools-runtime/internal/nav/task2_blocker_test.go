...
func (receiverTypeRunner) Run(_ context.Context, _ string, args ...string) ([]byte, error) {
	...
	types := `{"file":"src/main/java/Facade.java","range":{"start":{"line":0,"column":0},"end":{"line":30,"column":0}},"text":"public class Facade { }"}` + "\n"
	method := `{"file":"src/main/java/Facade.java","range":{"start":{"line":8,"column":0},"end":{"line":20,"column":0}},"text":"public void submit() { orderService.approve(); auditService.approve(); unknownService.approve(); }"}` + "\n"
	calls := `{"file":"src/main/java/Facade.java","range":{"start":{"line":10,"column":0},"end":{"line":10,"column":0}},"text":"orderService.approve()"}` + "\n" +
		`{"file":"src/main/java/Facade.java","range":{"start":{"line":11,"column":0},"end":{"line":11,"column":0}},"text":"auditService.approve()"}` + "\n" +
		`{"file":"src/main/java/Facade.java","range":{"start":{"line":12,"column":0},"end":{"line":12,"column":0}},"text":"unknownService.approve()"}` + "\n"
	switch {
	...
	case strings.Contains(p, "orderService"):
		return []byte(`{"file":"src/main/java/Facade.java","range":{"start":{"line":2,"column":0},"end":{"line":2,"column":0}},"text":"private final OrderService orderService;"}` + "\n"), nil
	case strings.Contains(p, "auditService"):
		return []byte(`{"file":"src/main/java/Facade.java","range":{"start":{"line":3,"column":0},"end":{"line":3,"column":0}},"text":"private final AuditService auditService;"}` + "\n"), nil
	}
	return nil, nil
}
...
func (springSyntaxRunner) Run(_ context.Context, _ string, args ...string) ([]byte, error) {
	...
	typ := `{"file":"src/main/java/OrderController.java","range":{"start":{"line":0,"column":0},"end":{"line":30,"column":0}},"text":"public class OrderController { }"}` + "\n"
	text := "@PostMapping(\n  value = \"/approve\"\n)\npublic Result<OrderVO> get(\n  @RequestParam(\"id\") Long id,\n  @RequestParam(required = false) String reason,\n  @PathVariable(\"orderId\") Long orderId) { return null; }"
	method := `{"file":"src/main/java/OrderController.java","range":{"start":{"line":4,"column":0},"end":{"line":12,"column":0}},"text":` + strconvQuote(text) + `}` + "\n"
	if len(args) > 0 && args[0] == "scan" {
		return []byte(typ + method), nil
	}
...
func TestGetSymbolInfoBalancesSpringParameterAnnotationsAndMultilineMapping(t *testing.T) {
	...
	if got.Signature != wantSig {
		t.Fatalf("signature=%q want=%q", got.Signature, wantSig)
	}
	if len(got.Annotations) != 1 || got.Annotations[0] != "@PostMapping( value = \"/approve\" )" {
