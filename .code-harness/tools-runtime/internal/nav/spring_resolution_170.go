...
func (n Navigator) ResolveInjection170(ctx context.Context, injection Injection170) (Relation170, error) {
	...
	if injection.Kind == "CONSTRUCTOR" && !springHasExplicitConstructor170(idx.FileMeta[owner.Path].Source, owner.Name) && springHasAnnotation170(owner.Imports, annotations(owner.Text), "RequiredArgsConstructor", "lombok.RequiredArgsConstructor") {
	...
	if selector == "" { selector = strings.TrimSpace(injection.ResourceName) }
	if selector != "" {
		filtered := make([]springCandidate170, 0)
		for _, candidate := range candidates {
			if candidate.BeanName == selector { filtered = append(filtered, candidate) }
		}
		selected = filtered
	}
	...
	if selector == "" && len(selected) > 1 {
		...
		for _, candidate := range selected {
			if candidate.Primary { primaries = append(primaries, candidate) }
		}
		if len(primaries) == 1 { selected = primaries } else if len(primaries) > 1 {
			for _, candidate := range primaries { rel.Targets = append(rel.Targets, candidate.Target) }
	...
	if len(selected) > 1 {
		for _, candidate := range selected { rel.Targets = append(rel.Targets, candidate.Target) }
		rel.Resolution = "AMBIGUOUS"
...
func (n Navigator) springCandidates170(idx *javaIndex170, declared, workspace, side string) ([]springCandidate170, error) {
	out := make([]springCandidate170, 0)
	for _, typ := range idx.Types {
		anns := annotations(typ.Text)
		if !springIsStereotype170(typ.Imports, anns) { continue }
		if !idx.springAssignable170(typ.FQCN, declared, map[string]bool{}) { continue }
	...
	for _, method := range idx.Methods {
		anns := annotations(method.Text)
		if !springHasRecognizedAnnotation170(method.Owner.Imports, anns, "Bean") { continue }
		returnType := idx.resolveTypeName170(method.Owner, method.ReturnType)
		if returnType == "" || !idx.springTypeCompatible170(returnType, declared) { continue }
		candidate := springCandidate170{
			BeanName:    springBeanName170(anns, method.Name),
			BeanType:    returnType,
			Primary:     springHasRecognizedAnnotation170(method.Owner.Imports, anns, "Primary"),
			Conditional: springHasCondition170(method.Owner.Imports, anns) || springHasCondition170(method.Owner.Imports, annotations(method.Owner.Text)),
	...
	sort.Slice(out, func(i, j int) bool {
		if out[i].BeanName != out[j].BeanName { return out[i].BeanName < out[j].BeanName }
		if out[i].Target.OwnerFQCN != out[j].Target.OwnerFQCN { return out[i].Target.OwnerFQCN < out[j].Target.OwnerFQCN }
...
func (idx *javaIndex170) springAssignable170(candidateFQCN, declared string, visiting map[string]bool) bool {
	if idx.springTypeCompatible170(candidateFQCN, declared) { return true }
	if visiting[candidateFQCN] { return false }
	...
	for _, iface := range typ.Interfaces {
		...
		if idx.springTypeCompatible170(fqcn, declared) || idx.springAssignable170(fqcn, declared, visiting) { return true }
	}
	if typ.Super != "" {
...
func springIsStereotype170(imports map[string]string, anns []string) bool {
	for _, name := range []string{"Service", "Component", "Repository", "Controller"} {
		if springHasRecognizedAnnotation170(imports, anns, name) { return true }
	}
	return false
}
...
func springHasRecognizedAnnotation170(imports map[string]string, anns []string, simple string) bool {
	for _, fqcn := range springAnnotationFQCN170[simple] {
		if springHasAnnotation170(imports, anns, simple, fqcn) { return true }
	}
	return false
}
...
func springHasAnnotation170(imports map[string]string, anns []string, simple, fqcn string) bool {
	for _, ann := range anns {
		...
		if i := strings.IndexByte(name, '('); i >= 0 { name = strings.TrimSpace(name[:i]) }
		if name == fqcn { return true }
		if name == simple && imports[simple] == fqcn { return true }
	}
	return false
}

func springHasCondition170(imports map[string]string, anns []string) bool {
	if springHasRecognizedAnnotation170(imports, anns, "Profile") || springHasRecognizedAnnotation170(imports, anns, "Conditional") { return true }
	for _, ann := range anns {
		name := strings.TrimPrefix(strings.TrimSpace(ann), "@")
		if i := strings.IndexByte(name, '('); i >= 0 { name = strings.TrimSpace(name[:i]) }
		simple := name
		if i := strings.LastIndexByte(simple, '.'); i >= 0 { simple = simple[i+1:] }
		if strings.HasPrefix(simple, "ConditionalOn") {
			fqcn := imports[simple]
			if strings.HasPrefix(fqcn, "org.springframework.boot.autoconfigure.condition.") || strings.HasPrefix(name, "org.springframework.boot.autoconfigure.condition.") { return true }
		}
	}
	return false
}
...
func springBeanName170(anns []string, fallback string) string {
	for _, ann := range anns {
		name := strings.TrimPrefix(strings.TrimSpace(ann), "@")
		simple := name
		if i := strings.IndexByte(simple, '('); i >= 0 { simple = simple[:i] }
		if i := strings.LastIndexByte(simple, '.'); i >= 0 { simple = simple[i+1:] }
		switch simple {
		case "Service", "Component", "Repository", "Controller", "Bean":
			if match := springQuotedValueRE170.FindStringSubmatch(ann); len(match) == 2 && strings.TrimSpace(match[1]) != "" { return strings.TrimSpace(match[1]) }
		}
	}
	if fallback == "" { return "" }
	runes := []rune(fallback)
...
func (n Navigator) springInjectionEvidence170(injection Injection170, owner *javaType170) (SourceRange170, error) {
	if len(injection.Evidence) > 0 {
		if err := validateSourceRange170(injection.Evidence[0]); err != nil { return SourceRange170{}, err }
		return injection.Evidence[0], nil
	}
	meta := owner.Text
	needle := injection.Name
	if injection.Kind == "CONSTRUCTOR" { needle = owner.Name }
	if injection.Kind == "SETTER" {
		runes := []rune(injection.Name)
		if len(runes) > 0 { runes[0] = []rune(strings.ToUpper(string(runes[0])))[0]; needle = "set" + string(runes) }
	}
	return n.springTextEvidence170(injection.Owner, owner.Path, meta, owner.Raw.StartLine, owner.Raw.StartColumn, needle)
}
...
func (n Navigator) springTextEvidence170(ref ReviewRef170, path, text string, startLine, startColumn int, needles ...string) (SourceRange170, error) {
	...
	needle := ""
	for _, candidate := range needles { if strings.TrimSpace(candidate) != "" { needle = candidate; break } }
	if needle == "" { needle = strings.TrimSpace(text); if len(needle) > 32 { needle = needle[:32] } }
...
func springUnresolved170(injection Injection170, evidence []SourceRange170, reason string) Relation170 {
	if evidence == nil { evidence = []SourceRange170{} }
