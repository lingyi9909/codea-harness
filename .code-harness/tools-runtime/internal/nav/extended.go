...
func methodParts(text,name string)(sig,ret string){line:=declarationLine(text);needle:=name+"(";i:=strings.Index(line,needle);if i<0{return "",""};open:=i+len(name);close:=balancedClose(line,open);if close<0{return "",""};sig=normalizeSpace(name+line[open:close+1]);before:=strings.TrimSpace(line[:i]);parts:=strings.Fields(before);if len(parts)>0{ret=parts[len(parts)-1]};return sig,ret}
