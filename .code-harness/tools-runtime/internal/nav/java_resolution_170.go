...
var (
	...
	// Stop before the terminating semicolon. This lets the same semicolon serve
	// as the delimiter for a following compact one-line import statement.
	javaImportRE170     = regexp.MustCompile(`(?m)(?:^|[;\n])\s*import\s+(?:static\s+)?([A-Za-z_$][A-Za-z0-9_$.]*)\b`)
...
func (n Navigator) buildJavaIndex170(ctx context.Context) (*javaIndex170, error) {
	...
	for _, raw := range methodRaw {
		method := javaMethodFromRaw170(idx, raw)
		if method == nil {
			continue
		}
		key := javaMethodRangeKey170(method)
		if seenMethods[key] {
			continue
		}
		seenMethods[key] = true
		idx.Methods = append(idx.Methods, method)
	}
	sortJavaMethods170(idx.Methods)
	return idx, nil
}
...
// parseLeadingAnnotations170 differs deliberately from the historical helper:
// an annotation without arguments ends at its qualified identifier, not at the
// end of the physical line. Java permits `@Service class A {}` and multiple
// annotations on the same line.
func parseLeadingAnnotations170(text string) ([]string, string) {
	...
	for strings.HasPrefix(s, "@") {
		...
		out = append(out, normalizeSpace(s[:end]))
		s = strings.TrimSpace(s[end:])
	}
	return out, s
}

func annotations170(text string) []string {
	items, _ := parseLeadingAnnotations170(text)
	return items
}

func declarationLine170(text string) string {
	_, rest := parseLeadingAnnotations170(text)
	if rest == "" {
		return ""
	}
...
func parseJavaSuper170(text string) string {
	match := javaExtendsRE170.FindStringSubmatch(declarationLine170(text))
	if len(match) != 2 {
		return ""
	}
...
func javaMethodFromRaw170(idx *javaIndex170, raw workspaceRawMatch) *javaMethod170 {
	...
	if name == "" {
		name = methodName170(raw.Text)
	}
	if name == "" {
		return nil
	}
	params, returnType := parseJavaMethodDeclaration170(raw.Text, name)
	return &javaMethod170{Owner: owner, Name: name, ReturnType: returnType, Params: params, Raw: raw, Text: raw.Text}
}

func methodName170(text string) string {
	line := declarationLine170(text)
...
func javaMethodRangeKey170(method *javaMethod170) string {
	return fmt.Sprintf("%s:%d:%d:%d:%d:%s:%s", method.Raw.Path, method.Raw.StartLine, method.Raw.StartColumn, method.Raw.EndLine, method.Raw.EndColumn, method.Owner.FQCN, method.Name)
}

func sortJavaMethods170(methods []*javaMethod170) {
...
func (n Navigator) InspectMethod170(ctx context.Context, ref ReviewRef170) (MethodFacts170, error) {
	...
	for _, call := range calls {
		...
		if call.Relation.Resolution != "EXACT" {
			...
			if len(call.Relation.Evidence) > 0 {
				at = call.Relation.Evidence[0]
			}
...
func (n Navigator) resolveMethodCalls170(ctx context.Context, idx *javaIndex170, method *javaMethod170, fromRef ReviewRef170) ([]javaResolvedCall170, error) {
	...
	for _, raw := range rawCalls {
		...
		if ownerFQCN == "" {
			...
			if receiverReason == "" {
				receiverReason = "JAVA_RECEIVER_UNRESOLVED"
			}
		...
		if len(candidates) == 0 {
			candidates, err = n.targetMethods170(ctx, idx, ownerFQCN, called)
			if err != nil {
				return nil, err
			}
		}
...
// targetMethods170 is a narrow AST query used only when the broad method index
// did not surface a declaration. It never guesses by name alone: the returned
// declaration must be inside the already-resolved receiver type's AST range.
func (n Navigator) targetMethods170(ctx context.Context, idx *javaIndex170, ownerFQCN, name string) ([]*javaMethod170, error) {
	typ := idx.ByFQCN[ownerFQCN]
	if typ == nil || !identRE.MatchString(name) {
		return []*javaMethod170{}, nil
	}
	raw, err := n.runWorkspaceRawTargets(ctx, []string{typ.Path}, workspaceMethodPatterns(name)...)
	if err != nil {
		return nil, err
	}
	seen := map[string]bool{}
	out := make([]*javaMethod170, 0)
	for _, match := range raw {
		method := javaMethodFromRaw170(idx, match)
		if method == nil || method.Owner.FQCN != ownerFQCN || method.Name != name {
			continue
		}
		key := javaMethodRangeKey170(method)
		if seen[key] {
...
func (idx *javaIndex170) methodsForCall170(ownerFQCN, name string) []*javaMethod170 {
	...
	walk = func(fqcn string) []*javaMethod170 {
		if fqcn == "" || visited[fqcn] {
			return nil
		}
		visited[fqcn] = true
		var methods []*javaMethod170
		for _, method := range idx.Methods {
			if method.Owner.FQCN == fqcn && method.Name == name {
...
func javaCallParts170(text string) (receiver, method string, args []string, ok bool) {
	...
	if argText == "" {
		return receiver, method, []string{}, true
	}
...
func (n Navigator) javaSourceRange170(ref ReviewRef170, raw workspaceRawMatch) (SourceRange170, error) {
	...
	// ast-grep's Java JSON columns are Unicode scalar columns. The workspace
	// adapter already made them one-based, so a UTF-8 byte conversion here
	// would corrupt columns after non-ASCII source text.
...
func (n Navigator) legacyMethodRef170(ctx context.Context, symbol, scope string) (ReviewRef170, *javaIndex170, error) {
	if err := n.validate(symbol, scope); err != nil {
		return ReviewRef170{}, nil, err
	}
	owner, name := splitSymbol(symbol)
	if name == "" {
		return ReviewRef170{}, nil, ErrInvalidSymbol
	}
	idx, err := n.buildJavaIndex170(ctx)
	if err != nil {
		return ReviewRef170{}, nil, err
	...
	if strings.Contains(owner, ".") {
		ownerFQCN = owner
	} else if matches := idx.SimpleFQCN[owner]; len(matches) == 1 {
		ownerFQCN = matches[0]
	}
	if ownerFQCN == "" {
		return ReviewRef170{}, nil, ErrAmbiguousSymbol
	}
	var candidates []*javaMethod170
	for _, method := range idx.Methods {
		if method.Owner.FQCN == ownerFQCN && method.Name == name {
			candidates = append(candidates, method)
		}
	}
	if len(candidates) == 0 {
		candidates, err = n.targetMethods170(ctx, idx, ownerFQCN, name)
		if err != nil {
			return ReviewRef170{}, nil, err
		}
	}
	if len(candidates) == 0 {
		return ReviewRef170{}, nil, ErrSymbolNotFound
	}
	if len(candidates) > 1 {
		return ReviewRef170{}, nil, ErrAmbiguousSymbol
	}
