...
func (routingRunner) Run(_ context.Context, _ string, args ...string) ([]byte, error) {
	...
	if len(args) > 0 && args[0] == "scan" {
		return []byte(typeLine + enumLine + methodLine + callerType + callerMethod), nil
	}
	switch {
	...
	case strings.Contains(p, "@PostMapping"):
		return []byte(methodLine), nil
	case strings.Contains(p, " service;") || strings.Contains(p, " service ="):
		return []byte(`{"file":"src/main/java/OrderFacade.java","range":{"start":{"line":2,"column":0},"end":{"line":2,"column":0}},"text":"private final OrderService service;"}` + "\n"), nil
...
func TestN5Callers(t *testing.T) {
	...
	if len(got.Callers) != 1 || got.Callers[0].CallerSymbol != "OrderFacade.submit" || got.Callers[0].Line != 7 || got.Callers[0].Resolution != "CONFIRMED" {
		t.Fatalf("got=%+v", got)
	}
}
...
func (ambiguousRunner) Run(_ context.Context, _ string, args ...string) ([]byte, error) {
	...
	typ := `{"file":"src/main/java/A.java","range":{"start":{"line":0,"column":0},"end":{"line":20,"column":0}},"text":"class OrderController {}"}` + "\n"
	methods := `{"file":"src/main/java/A.java","range":{"start":{"line":3,"column":0},"end":{"line":5,"column":0}},"text":"Result<Void> approve(A a) {}"}` + "\n" + `{"file":"src/main/java/A.java","range":{"start":{"line":7,"column":0},"end":{"line":9,"column":0}},"text":"Result<Void> approve(B b) {}"}` + "\n"
	if len(args) > 0 && args[0] == "scan" {
		return []byte(typ + methods), nil
	}
	if strings.Contains(p, "OrderController") {
		return []byte(typ), nil
	}
	if strings.Contains(p, "approve($$$ARGS)") {
		return []byte(methods), nil
	}
	return nil, nil
}
...
func (fieldRunner) Run(_ context.Context, _ string, args ...string) ([]byte, error) {
	...
	typ := `{"file":"src/main/java/A.java","range":{"start":{"line":0,"column":0},"end":{"line":20,"column":0}},"text":"class OrderController {}"}` + "\n"
	field := `{"file":"src/main/java/A.java","range":{"start":{"line":2,"column":0},"end":{"line":2,"column":0}},"text":"private static final int DEFAULT_LIMIT = 100;"}` + "\n"
	if len(args) > 0 && args[0] == "scan" {
		return []byte(typ + field), nil
	}
	if strings.Contains(p, "OrderController") {
		return []byte(typ), nil
	}
