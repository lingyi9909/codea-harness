...
func verifySelectionTurnBytesForRoot180(data []byte, root string, req SelectionTurnRequest180) error {
    if strings.TrimSpace(req.SessionID)==""||strings.TrimSpace(req.MessageID)==""||strings.TrimSpace(req.RunID)==""||strings.TrimSpace(req.OptionsHash)=="" { return errors.New("HUMAN_SELECTION_REQUIRED: missing Host/menu identity") }
