...
func Test170DispatchRequiresDiscoveryForNewRun(t *testing.T) {
	...
	for _, stage := range []string{reviewprogress.StageSnapshot, reviewprogress.StageChangeAnalysis, reviewprogress.StageCertification} {
		if _, err := reviewprogress.Advance(".", runID, stage); err != nil {
			t.Fatal(err)
		}
	}
...
func Test170BaseAdapterReturnsDeletedMethodAndMapperEvidence(t *testing.T) {
	if os.Getenv("CODEA_AST_GREP") == "" {
		t.Fatal("CODEA_AST_GREP must be provisioned")
	}
	root := t.TempDir()
	...
	snap, err := changeset.Compute(root, base, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, ref := range refs {
		if ref.Name == "removed" {
			foundRemoved = true
			removedRange = ranges[i]
		}
		if ref.Side != "BASE" {
			t.Fatalf("base ref escaped as %s", ref.Side)
		}
	}
	if !foundRemoved {
		t.Fatal("deleted method missing from BASE adapter")
	}
	_ = removedRange
	var mapperRange nav.SourceRange170
	for i, ref := range refs {
		if ref.Name == "updateStatus" {
			mapperRange = ranges[i]
		}
	}
	if mapperRange.Ref.Name == "" {
		t.Fatal("BASE mapper method missing")
	}
	...
	for _, rel := range rels {
		if rel.Kind != "MYBATIS_STATEMENT" || rel.Resolution != "EXACT" {
			continue
		}
		for _, ev := range rel.Evidence {
			if ev.Ref.Side == "BASE" && strings.HasSuffix(ev.Ref.Path, "OrderMapper.xml") {
				foundTenantBase = true
			}
		}
	}
	if !foundTenantBase {
		t.Fatalf("BASE statement evidence missing: %+v", rels)
	}
}
...
func Test170UseTimeVerificationRejectsTamperedRuleContext(t *testing.T) {
	...
	target := seed
	target.Path = "src/main/java/demo/B.java"
	target.OwnerFQCN = "demo.B"
	target.Name = "save"
	...
	claimed, err := reviewcontext.Build170(context.Background(), input, resolver)
	if err != nil {
		t.Fatal(err)
	}
...
func (r *useTimeResolver170) Method(context.Context, nav.ReviewRef170) (nav.MethodFacts170, error) {
	return r.facts, nil
}
func (r *useTimeResolver170) Callers(context.Context, nav.ReviewRef170) ([]nav.Relation170, error) {
	return []nav.Relation170{}, nil
}
func (r *useTimeResolver170) Mapper(context.Context, nav.SourceRange170) ([]nav.Relation170, []nav.Issue170, error) {
	return []nav.Relation170{}, []nav.Issue170{}, nil
}
func (r *useTimeResolver170) Dubbo(context.Context, nav.ReviewRef170) ([]nav.Relation170, []nav.Issue170, error) {
	return []nav.Relation170{}, []nav.Issue170{}, nil
}
func (r *useTimeResolver170) Spring(context.Context, nav.ReviewRef170) ([]nav.Relation170, []nav.Issue170, error) {
	return []nav.Relation170{}, []nav.Issue170{}, nil
}
