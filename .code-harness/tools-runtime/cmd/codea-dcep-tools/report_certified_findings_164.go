...
func applyCertifiedFindingAuthority164(runID string, analysisCert analysisruntime.Certificate, authoritative *report.ReviewRequest) error {
	...
	finalized, err := report.FinalizeCertifiedRequest170(*authoritative, set)
	if err != nil {
		return fmt.Errorf("REVIEWER_UNAVAILABLE: MANUAL_ACTION_REQUIRED: HARD STOP: invalid Runtime-certified review context: %w", err)
	}
	*authoritative = finalized
	return nil
}
