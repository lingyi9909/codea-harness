...
func decodeReviewContextRequest170(pathRunID string, raw []byte) (reviewContextRequest170, error) {
	...
	if err := dec.Decode(&req); err != nil { return req, fmt.Errorf("REVIEW_CONTEXT_REQUEST_INVALID: %w", err) }
	var extra any
	if err := dec.Decode(&extra); !errors.Is(err, os.ErrClosed) && err == nil { return req, fmt.Errorf("REVIEW_CONTEXT_REQUEST_INVALID: trailing JSON") }
	req.RunID = strings.TrimSpace(req.RunID)
	if req.RunID == "" || req.RunID != strings.TrimSpace(pathRunID) { return req, fmt.Errorf("RUN_ID_MISMATCH: body=%q path=%q", req.RunID, pathRunID) }
	req.Phase = strings.ToUpper(strings.TrimSpace(req.Phase))
	if req.Phase != "DISCOVERY" && req.Phase != "RULES" { return req, fmt.Errorf("REVIEW_CONTEXT_PHASE_INVALID: %q", req.Phase) }
	return req, nil
}

func validateReviewContextProgress170(state reviewprogress.State, phase string) error {
	if state.Status != reviewprogress.StatusRunning { return fmt.Errorf("REVIEW_CONTEXT_PROGRESS_TERMINAL: %s", state.Status) }
	switch strings.ToUpper(strings.TrimSpace(phase)) {
	case "DISCOVERY":
		if state.CurrentStage != reviewprogress.StageChangeAnalysis { return fmt.Errorf("REVIEW_CONTEXT_PHASE_ILLEGAL: DISCOVERY requires CHANGE_ANALYSIS, current=%s", state.CurrentStage) }
	case "RULES":
		if state.CurrentStage != reviewprogress.StageReviewPlanning { return fmt.Errorf("REVIEW_CONTEXT_PHASE_ILLEGAL: RULES requires REVIEW_PLANNING, current=%s", state.CurrentStage) }
	default:
...
func runReviewContext170(args []string) error {
	...
	if fs.NArg() != 0 || strings.TrimSpace(*inputPath) == "" { return errors.New("review context requires --input") }
	runID, cleanInput, err := validateAnalysisRequestPath153(*inputPath)
	if err != nil { return errors.New("review context input must be under .code-harness/runs/<runId>/requests") }
	if err := verifyReviewTransportPath153(runID, cleanInput); err != nil { return err }
	raw, err := os.ReadFile(cleanInput)
	if err != nil { return fmt.Errorf("REVIEW_CONTEXT_REQUEST_READ_FAILED: %w", err) }
	req, err := decodeReviewContextRequest170(runID, raw)
	if err != nil { return err }
	state, err := reviewprogress.Read(".", runID)
	if err != nil { return err }
	if err := validateReviewContextProgress170(state, req.Phase); err != nil { return err }
	if _, err := os.Stat(reviewContextMarker170(runID)); err != nil { return fmt.Errorf("REVIEW_CONTEXT_RUN_NOT_170: %w", err) }
	...
	if err != nil { return fmt.Errorf("REVIEW_CONTEXT_ANALYSIS_UNAVAILABLE: %w", err) }
	...
	if req.Phase == "RULES" {
		...
		if err != nil { return fmt.Errorf("REVIEW_CONTEXT_RULES_UNITS_UNAVAILABLE: %w", err) }
		dispatch, err := loadRuleDispatch170(dispatchPath)
		if err != nil { return err }
		if dispatch.RunID != runID || dispatch.ReviewUnitsSHA256 != units.SHA256 { return fmt.Errorf("REVIEW_CONTEXT_RULES_DISPATCH_STALE") }
	...
	if err != nil { return err }
	if err := reviewcontext.VerifyRelations170(context.Background(), built, buildInput, resolver); err != nil { return err }
	encoded, err := json.MarshalIndent(built, "", "  ")
	if err != nil { return fmt.Errorf("REVIEW_CONTEXT_ENCODE_FAILED: %w", err) }
	encoded = append(encoded, '\n')
	artifactPath := filepath.Join(".code-harness", "runs", runID, "analysis", "review-context-"+strings.ToLower(req.Phase)+".json")
	if err := atomicReviewWrite153(artifactPath, encoded); err != nil { return fmt.Errorf("REVIEW_CONTEXT_WRITE_FAILED: %w", err) }
	if req.Phase == "RULES" {
		if err := advanceReviewProgressStage164(runID, reviewprogress.StageReviewPlanning, unitsPath, dispatchPath, filepath.ToSlash(artifactPath)); err != nil { return err }
	}
...
func newRuntimeResolver170(root string) (*runtimeResolver170, error) {
	...
	if err != nil { return nil, fmt.Errorf("REVIEW_CONTEXT_WORKSPACE_CONFIG_READ_FAILED: %w", err) }
	...
	for _, result := range verified {
		if result.Status != workspace.StatusVerified || strings.TrimSpace(result.ConfirmedRoot) == "" { continue }
...
func (r *runtimeResolver170) navigator(ref nav.ReviewRef170) (nav.Navigator, error) {
	...
	if !ok { return nav.Navigator{}, fmt.Errorf("PROVIDER_SOURCE_UNAVAILABLE: workspace %s is not verified", ref.Workspace) }
	return n, nil
}
func (r *runtimeResolver170) Method(ctx context.Context, ref nav.ReviewRef170) (nav.MethodFacts170,error) {
	n, err := r.navigator(ref); if err != nil { return nav.MethodFacts170{}, err }
...
func (r *runtimeResolver170) Mapper(ctx context.Context, source nav.SourceRange170) ([]nav.Relation170,[]nav.Issue170,error) {
	n, err := r.navigator(source.Ref); if err != nil { return nil,nil,err }
...
func (r *runtimeResolver170) Dubbo(ctx context.Context, ref nav.ReviewRef170) ([]nav.Relation170,[]nav.Issue170,error) {
	n, err := r.navigator(ref); if err != nil { return nil,nil,err }
...
func loadRuleDispatch170(path string) (reviewrules.Manifest,error) {
	raw, err := os.ReadFile(filepath.FromSlash(path)); if err != nil { return reviewrules.Manifest{}, fmt.Errorf("REVIEW_CONTEXT_RULES_DISPATCH_UNAVAILABLE: %w", err) }
	var out reviewrules.Manifest
	dec := json.NewDecoder(bytes.NewReader(raw)); dec.DisallowUnknownFields()
	if err := dec.Decode(&out); err != nil { return out, fmt.Errorf("REVIEW_CONTEXT_RULES_DISPATCH_INVALID: %w", err) }
	return out,nil
}
...
func seedsFromAnalysis170(a analysisruntime.ChangeAnalysis) []nav.ReviewRef170 {
	...
	add := func(workspaceID, pathValue, symbol string) {
		...
		if workspaceID != "current" || !strings.HasSuffix(strings.ToLower(pathValue), ".java") { return }
		ref, ok := methodRefFromSymbol170(workspaceID, pathValue, symbol); if !ok { return }
	...
	for _, chain := range a.CallChains {
		for i, symbol := range chain.Chain {
			if i < len(chain.ChainRefs) { add(chain.ChainRefs[i].Workspace, chain.ChainRefs[i].Path, symbol) }
		}
	}
	for _, loc := range a.SymbolLocations { add(loc.Workspace, loc.Path, loc.Symbol) }
	sort.Slice(refs, func(i,j int) bool { ki,_:=nav.ReviewRefKey170(refs[i]); kj,_:=nav.ReviewRefKey170(refs[j]); return ki<kj })
	return refs
}
...
func methodRefFromSymbol170(workspaceID, p, symbol string) (nav.ReviewRef170,bool) {
	i := strings.LastIndex(strings.TrimSpace(symbol), "."); if i <= 0 || i+1 >= len(symbol) { return nav.ReviewRef170{},false }
	owner, name := symbol[:i], symbol[i+1:]
	ownerSimple := owner; if j:=strings.LastIndex(ownerSimple,"."); j>=0 { ownerSimple=ownerSimple[j+1:] }
	ownerFQCN := owner
	if workspaceID == "current" && !strings.Contains(owner, ".") {
		if raw, err := os.ReadFile(filepath.FromSlash(p)); err == nil { if m:=package170.FindSubmatch(raw); len(m)==2 { ownerFQCN=string(m[1])+"."+ownerSimple } }
	}
...
func needsFromDispatch170(units reviewunit.Manifest, dispatch reviewrules.Manifest, a analysisruntime.ChangeAnalysis) []reviewcontext.Need170 {
	unitByID := map[string]reviewunit.Unit{}; for _, u := range units.Units { unitByID[u.ID]=u }
	needs := []reviewcontext.Need170{}
	for _, d := range dispatch.Dispatches {
		u, ok := unitByID[d.ReviewUnitID]; if !ok { continue }
		seeds := []nav.ReviewRef170{}
		for i, symbol := range u.Chain {
			p := ""; workspaceID := "current"
			if i < len(u.Files) { p=u.Files[i].Path; workspaceID=u.Files[i].Workspace }
			if p=="" { for _, loc := range a.SymbolLocations { if loc.Symbol==symbol { p=loc.Path; workspaceID=loc.Workspace; break } } }
			if ref, ok := methodRefFromSymbol170(workspaceID,p,symbol); ok { seeds=append(seeds,ref) }
		}
...
func reviewContextMarker170(runID string) string { return filepath.Join(".code-harness","runs",runID,"runtime","review-context-170.enabled") }
