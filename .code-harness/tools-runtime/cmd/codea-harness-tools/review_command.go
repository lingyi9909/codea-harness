...
func runReviewOptions(args []string) error {
	...
	if err != nil { return fmt.Errorf("read review options request: %w", err) }
	var req reviewOptionsRequest
	if err := decodeStrictChainRequest(requestBytes, &req, "review options request"); err != nil { return err }
	...
	if err != nil { return err }
	if err := validateReviewContract153("review-options.schema.json", optionsBytes); err != nil { return err }
	artifactPath := filepath.Join(".code-harness", "runs", req.RunID, "analysis", "review-options.json")
	if err := atomicReviewWrite153(artifactPath, optionsBytes); err != nil { return err }
...
func runReviewSelect(args []string) error {
	...
	if err != nil { return err }
	requestBytes, err := os.ReadFile(cleanInput)
	if err != nil { return fmt.Errorf("read review selection request: %w", err) }
	if err := validateReviewContract153("review-selection-request.schema.json", requestBytes); err != nil { return err }
	var req reviewselection.SelectionRequest
	if err := decodeStrictChainRequest(requestBytes, &req, "review selection request"); err != nil { return err }
	if req.RunID != pathRunID || !validChainArtifactID(req.RunID) {
		return fmt.Errorf("RUN_ID_PATH_MISMATCH: body runId %q does not match request path runId %q", req.RunID, pathRunID)
	}
	...
	if err != nil { return err }
	if err := validateReviewContract153("review-scope.schema.json", scopeBytes); err != nil { return fmt.Errorf("REVIEW_SELECTION_SCOPE_INVALID: %w", err) }
	artifactPath := filepath.Join(".code-harness", "runs", req.RunID, "analysis", "review-scope.json")
	if err := atomicReviewWrite153(artifactPath, scopeBytes); err != nil { return err }
...
func canonicalReviewJSON153(value any) ([]byte, error) {
	...
	if err != nil { return nil, fmt.Errorf("REVIEW_ARTIFACT_ENCODE_FAILED: %w", err) }
	return append(data, '\n'), nil
}
...
func validateReviewContract153(name string, data []byte) error {
	...
	if err != nil { return fmt.Errorf("REVIEW_CONTRACT_READ_FAILED: %s: %w", name, err) }
...
func atomicReviewWrite153(path string, data []byte) error {
	...
	if err != nil { return fmt.Errorf("REVIEW_ARTIFACT_TEMP_FAILED: %w", err) }
	tmpPath := tmp.Name()
	cleanup := func() { _ = os.Remove(tmpPath) }
	if _, err := tmp.Write(data); err != nil { _ = tmp.Close(); cleanup(); return fmt.Errorf("REVIEW_ARTIFACT_WRITE_FAILED: %w", err) }
	if err := tmp.Close(); err != nil { cleanup(); return fmt.Errorf("REVIEW_ARTIFACT_WRITE_FAILED: %w", err) }
	if err := os.Rename(tmpPath, path); err != nil { cleanup(); return fmt.Errorf("REVIEW_ARTIFACT_REPLACE_FAILED: %w", err) }
	return nil
}
